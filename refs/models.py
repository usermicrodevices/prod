...
class PrintTemplates(CustomAbstractModel):
    ...
    content = models.TextField(null=False, blank=False, default='''<table class="page-pad" style="border:1px solid black;margin-left:auto;margin-right:auto;border-collapse:collapse;"><caption>{{doc.type.name}} N{{doc.id}} from {{doc.registered_at}}<p style="text-align:left;margin:0;padding:0;">seler: {{doc.owner}}</p><p style="text-align:left;margin:0;padding:0;">purchaser: {{doc.contractor.name}}</p></caption><thead><tr><th style="width:5%;border:2px solid black;">number</th><th style="width:15%;border:2px solid black;">article</th><th style="width:50%;border:2px solid black;">product</th><th style="width:10%;border:2px solid black;">count</th><th style="width:10%;border:2px solid black;">price</th><th style="width:10%;border:2px solid black;">sum</th></tr></thead><tbody>{% for r in records %}<tr><td style="border:2px solid black;">{{forloop.counter}}</td><td style="border:2px solid black;">{{r.product.article}}</td><td style="border:2px solid black;">{%if 'label' in r.product.extinfo%}{{r.product.extinfo.label}}{%else%}{{r.product.name}}{%endif%}</td><td style="border:2px solid black;">{{r.count}}</td><td style="border:2px solid black;">{{r.price}}</td><td style="border:2px solid black;">{{r.sum_price}}</td></tr>{% endfor %}<tr><td style="border:2px solid black;text-align:right;" colspan=6>Total: {{doc.sum_final}}</td></tr></tbody></table>''', verbose_name=_('content'))
